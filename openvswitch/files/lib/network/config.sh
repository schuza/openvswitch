		...
		case "$iftype" in
			interface)
				...
				case "$iftype" in
					...
					ovs)
						config_set "$CONFIG_SECTION" ifnames "$device"
						config_set "$CONFIG_SECTION" ifname ovs-"$CONFIG_SECTION"
					;;
				esac
...
prepare_interface() {
	...
	[ "br-$config" = "$iface" -o -e "$iface" -o "ovs-$config" = "$iface" ] && return 0;
	...
	case "$iftype" in
		...
		ovs)
			...
			config_get macaddr "$config" macaddr
			[ -x /usr/bin/ovs-vsctl ] && {
				ifconfig "ovs-$config" 2>/dev/null >/dev/null && {
					local newdevs devices
					config_get devices "$config" device
					for dev in $(sort_list "$devices" "$iface"); do
						append newdevs "$dev"
					...
					$DEBUG ifconfig "$iface" 0.0.0.0
					$DEBUG do_sysctl "net.ipv6.conf.$iface.disable_ipv6" 1
					$DEBUG ovs-vsctl add-port "ovs-$config" "$iface"
					# Bridge existed already. No further processing necesary
				} || {
					local stp
					config_get_bool stp "$config" stp 0
					$DEBUG ovs-vsctl add-br "ovs-$config"
					# Wait for bridge to go into forwarding state
					sleep 1
					$DEBUG ifconfig "$iface" 0.0.0.0
					$DEBUG do_sysctl "net.ipv6.conf.$iface.disable_ipv6" 1
					$DEBUG ovs-vsctl add-port "ovs-$config" "$iface"
					...
					# select datapath ID
					local datapathid
					config_get datapathid $config datapathid
					#datapathid="0400$(cat /sys/class/net/eth0/address  | sed -e 's/://g')"
					[ -n "$datapathid" ] && ovs-vsctl set bridge "ovs-$config" other-config:datapath-id=$datapathid

					# set up a connection to an openflow controller
					local controller ofmode ofport failmode
					config_get controller $config controller
					config_get ofmode $config ofmode
					config_get ofport $config ofport
					config_get failmode $config failmode
					[ -n "$ofport" ] && ofport=":$ofport"
					#ofport=${ofport:-6677}

					if [ -n "$controller" ]
					then
						ovs-vsctl set-controller "ovs-$config" "$ofmode:$controller$ofport"
						[ -n "$failmode" ] && ovs-vsctl set-fail-mode "ovs-$config" "$failmode"
					fi

					# set up GRE tunneling
					local gre
					config_get gre $config gre
					[ -n "$gre" ] && ovs-vsctl add-port "ovs-$config" gre0 -- set Interface gre0 type=gre options:remote_ip=$gre

					[ -z "$macaddr" -a -e "/sys/class/net/$iface/address" ] && macaddr="$(cat /sys/class/net/$iface/address)"
					$DEBUG ifconfig "ovs-$config" ${macaddr:+hw ether "${macaddr}"} up
					# Add flow rule for 802.1x packets
					$DEBUG ovs-ofctl add-flow "ovs-$config" dl_type=0x888e,actions=LOCAL
		...
		;;
	#	*)
	#		( type "prepare_interface_$iftype" ) >/dev/null 2>/dev/null && eval "prepare_interface_$iftype '$iface' '$config' '$vifmac'"
	#	;;
...
setup_interface() {
	...
	[ "$iface" = "br-$config" -o "$iface" = "ovs-$config" ] && {
